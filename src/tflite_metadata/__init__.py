...
# https://github.com/tensorflow/tflite-support/blob/v0.4.3/tensorflow_lite_support/metadata/python/metadata.py
# without dependencies on generated code that is platform dependent.
...
from tflite.BertTokenizerOptions import BertTokenizerOptionsT
from tflite.Buffer import BufferT
from tflite.Metadata import MetadataT
from tflite.Model import Model, ModelT
from tflite.ModelMetadata import ModelMetadata, ModelMetadataT
from tflite.RegexTokenizerOptions import RegexTokenizerOptionsT
from tflite.SentencePieceTokenizerOptions import SentencePieceTokenizerOptionsT

try:
    # If exists, optionally use TensorFlow to open and check files. Used to
    # support more than local file systems.
    # In pip requirements, we don't necessarily need tensorflow as a dep.
...
_FLATC_TFLITE_METADATA_SCHEMA_FILE = get_path_to_datafile("schema/metadata_schema.fbs")
...
class MetadataPopulator(object):
    ...
    def get_recorded_associated_file_list(self):
        ...
        metadata = ModelMetadataT.InitFromObj(
            ModelMetadata.GetRootAsModelMetadata(self._metadata_buf, 0)
        )
    ...
    def load_associated_file_buffers(self, associated_files):
        ...
        self._associated_files.update(
            {os.path.basename(name): buffers for name, buffers in associated_files.items()}
        )
    ...
    def load_metadata_buffer(self, metadata_buf):
        ...
        # https://github.com/tensorflow/tflite-support/blob/master/tensorflow_lite_support/metadata/cc/metadata_version.cc#L302
        min_version = "1.0.0"

        # Inserts in the minimum metadata parser version into the metadata_buf.
        metadata = ModelMetadataT.InitFromObj(ModelMetadata.GetRootAsModelMetadata(metadata_buf, 0))
    ...
    def load_metadata_and_associated_files(self, src_model_buf):
        ...
        if _is_zipfile(io.BytesIO(src_model_buf)):
            with _open_as_zipfile(io.BytesIO(src_model_buf)) as zf:
                self.load_associated_file_buffers({f: zf.read(f) for f in zf.namelist()})
    ...
    def _copy_archived_files(self, src_zip, file_list, dst_zip):
        ...
        with _open_as_zipfile(src_zip, "r") as src_zf, _open_as_zipfile(dst_zip, "a") as dst_zf:
            src_list = src_zf.namelist()
            for f in file_list:
                if f not in src_list:
                    raise ValueError(
                        "File, '{0}', does not exist in the zipfile, {1}.".format(f, src_zip)
                    )
    ...
    def _get_associated_files_from_process_units(self, table, field_name):
        ...
        for process_unit in process_units or []:
            ...
            if isinstance(options, (BertTokenizerOptionsT, RegexTokenizerOptionsT)):
                file_list += self._get_associated_files_from_table(options, "vocabFile")
            elif isinstance(options, SentencePieceTokenizerOptionsT):
                file_list += self._get_associated_files_from_table(options, "sentencePieceModel")
                file_list += self._get_associated_files_from_table(options, "vocabFile")
        return file_list
    ...
    def _get_recorded_associated_file_object_list(self, metadata):
        ...
        recorded_files += self._get_associated_files_from_table(metadata, "associatedFiles")

        # Add associated files attached to each SubgraphMetadata.
        for subgraph in metadata.subgraphMetadata or []:
            recorded_files += self._get_associated_files_from_table(subgraph, "associatedFiles")
    ...
    def _populate_metadata_buffer(self):
        ...
        model = ModelT.InitFromObj(Model.GetRootAsModel(model_buf, 0))
        buffer_field = BufferT()
    ...
    def _validate_metadata(self, metadata_buf):
        ...
        model_meta = ModelMetadata.GetRootAsModelMetadata(metadata_buf, 0)
        if model_meta.SubgraphMetadataLength() != 1:
            raise ValueError(
                "The number of SubgraphMetadata should be exactly one, "
                "but got {0}.".format(model_meta.SubgraphMetadataLength())
            )
        ...
        if num_input_tensors != num_input_meta:
            raise ValueError(
                ...
                "input tensor metadata ({1})".format(num_input_tensors, num_input_meta)
            )
        num_output_tensors = model.Subgraphs(0).OutputsLength()
        num_output_meta = model_meta.SubgraphMetadata(0).OutputTensorMetadataLength()
        if num_output_tensors != num_output_meta:
            raise ValueError(
                "The number of output tensors ({0}) should match the number of "
                "output tensor metadata ({1})".format(num_output_tensors, num_output_meta)
            )
...
class MetadataDisplayer(object):
    ...
    def get_associated_file_buffer(self, filename):
        ...
        if filename not in self._associated_file_list:
            raise ValueError("The file, {}, does not exist in the model.".format(filename))
...
def _assert_model_buffer_identifier(model_buf):
    if not Model.ModelBufferHasIdentifier(model_buf, 0):
        raise ValueError(
...
def _assert_metadata_buffer_identifier(metadata_buf):
    ...
    if not ModelMetadata.ModelMetadataBufferHasIdentifier(metadata_buf, 0):
        raise ValueError(
